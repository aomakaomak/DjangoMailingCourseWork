...
class SendMail(models.Model):
    ...
    first_mailing = models.DateTimeField(verbose_name='Дата и время первой отправки', blank=True, null=True)
    last_mailing = models.DateTimeField(verbose_name='Дата и время окончания отправки', blank=True, null=True)
...
class MailingAttempt(models.Model):
    ...
    status = models.CharField(max_length=8, choices=ATTEMPT_STATUS_CHOICES, verbose_name='Статус')
    answer = models.TextField(verbose_name='Ответ почтового сервиса')
    sendmail = models.ForeignKey(SendMail, on_delete=models.CASCADE, related_name='attempts', verbose_name='Рассылка')
